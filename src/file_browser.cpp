...
namespace openflash
{
    ...
    bool file_browser::render_file_list()
    {
        ...
        int lowerBoundary = (_browser_state.current_file_index /
                             max_file_count_pagination) *
                            max_file_count_pagination;

        int upperBoundary = bn::min(lowerBoundary +
                                        max_file_count_pagination,
                                    _current_depth_files.size());
        ...
        for (int index = lowerBoundary; index < upperBoundary; ++index)
        {
            ...
            if (file.is_folder())
                ...
                _icons.push_back(bn::sprite_items::save.create_sprite(file_x - 12, file_y + (index - lowerBoundary) * text_spacing_y));

            _icons.back().set_bg_priority(0);
